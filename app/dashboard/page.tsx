...
// --- FIXED IMPORT PATH (ROOT-LEVEL LIB) ---
...
function IntegrationTest() {
  ...
  // Execute Token Exchange + RLS Query
  useEffect(() => {
  ...
  const renderStatus = () => {
    ...
    return (
      <div
        className={`p-5 rounded-xl shadow-lg text-left ${
          isAuthenticated
  ...
  return (
    <main className="min-h-screen w-full bg-gradient-to-b from-[#ffffff] via-[#fff6f6] to-[#fff0f2] flex flex-col items-center text-[#0A0A0A] px-6 py-14 sm:py-20">
      <div className="w-full max-w-6xl">
        ...
        {/* Supabase/Outseta Integration Status */}
        <div className="mb-14">
          ...
          {sections.map((section, i) => (
            ...
            >
              <div className="bg-white rounded-3xl border border-gray-100 shadow-sm hover:shadow-[0_8px_35px_rgba(0,0,0,0.07)] transition-all duration-300 p-8 flex flex-col gap-2 cursor-pointer">
                ...
                <p className="text-gray-600 text-[15px] leading-snug">{section.desc}</p>
              </div>
            </a>
          ))}
        </div>
              ...
              <a key={i} href={platform.href}>
                <div className="bg-white rounded-3xl border border-gray-100 p-8 text-left shadow-sm hover:shadow-[0_8px_35px_rgba(0,0,0,0.07)] transition-all duration-300">
                  <div className="flex items-center gap-4 mb-4">
                    <img
                      src={`https://placehold.co/42x42/CBAF7A/ffffff?text=${platform.name.slice(
                        0,
                        1
                      )}`}
                      alt={platform.name}
                    ...
                    />
                    <h3 className="text-xl font-semibold text-[#0a0a0a]">{platform.name}</h3>
                  </div>
                  <p className="text-gray-600 text-[15px] leading-relaxed">{platform.desc}</p>
